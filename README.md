...
- **Validar = pago:** ao validar um mês, o app grava o valor confirmado **e cria o lançamento real na `DADOS`** (com a data de referência daquele mês). O vínculo é guardado em `FIXAS_PROJECAO.LANC_ID`, então revalidar **edita** (não duplica) e **Desfazer** remove o lançamento. Registrar uma despesa já com o toggle "fixa" ligado marca o mês como validado e vinculado automaticamente.
